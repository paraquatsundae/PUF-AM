...
**Status:** Plan written 2026-09-10. Phase 1 slices A–C built 2026-09-10/11 (§5); slice C's rules change awaits `npm run deploy:rules` and the live A→B exit check. Phase 2's hermetic part built 2026-09-11 (native PUT everywhere, `fdev` and Hyphanet FCP gone, node pinned 0.2.135); its live check (`npm run mist:smoke:native`) is pending. **2026-09-14:** Phase 4 partial — three-device Bones paddock sync (bake **0.0.2**); Hole 5 not closed. **Next (2026-09-15):** [`DAY_RUN_2026_09_15.md`](DAY_RUN_2026_09_15.md). Supersedes the direction in the docs listed under § Supersedes.
...
Related: [`APK_FREENET_HOST.md`](APK_FREENET_HOST.md) (Android host, becomes Phase 3 here) · [`FREENET_OPERATOR_FLOW.md`](FREENET_OPERATOR_FLOW.md) §8–§9 · [`LOGIN_JOIN_SINGLE_BOX.md`](LOGIN_JOIN_SINGLE_BOX.md) **Decision — 2026-09-12** (join UX / hole 2) · [`CROP_PACK_PLUGIN.md`](CROP_PACK_PLUGIN.md) · [`PLUGIN_AUTHORING.md`](PLUGIN_AUTHORING.md) · [`FIREBASE_BILLING.md`](FIREBASE_BILLING.md) · [`NAMING.md`](NAMING.md) §1 · reference: [`reference/DESKTOP_FREENET_PLUGIN.md`](reference/DESKTOP_FREENET_PLUGIN.md), [`reference/APK_FREENET_PLUGIN.md`](reference/APK_FREENET_PLUGIN.md), [`reference/MIST_NETWORK_STORAGE.md`](reference/MIST_NETWORK_STORAGE.md)
...
**Decision — 2026-09-12 (map highlights).** Timed “check this” map areas travel in **Hot** (`map_highlight` records, local kind `map_highlights`) so a crew device with Hot/Bones keys can decrypt them. Not a FarmSeed-only slot. Snapshot-on-Send, same as diary — see [`FREENET_OPERATOR_FLOW.md`](FREENET_OPERATOR_FLOW.md) §9.2.

**Decision — 2026-09-12 (Hot watch).** After the first Send, highlight (and Hot diary) updates **auto-publish** from the painting terminal and other terminals **poll a HotKey watch slot** (20 s; no Freenet subscribe in 0.2.135). Ping is generation/hash; yes fetches the new Hot URI. Crew never receive FarmSeed. Hole 4: revoke ≠ kick. See [`SETTINGS_SYNC_AND_CREW.md`](SETTINGS_SYNC_AND_CREW.md) §9.

**Decision — 2026-09-13 (Bones watch).** After the first Send, paddock / pin / track saves **auto-publish Bones** (BonesKey only — crew or owner; never FarmSeed) and bump the **same** 20 s watch slot with `bonesUri` + `bonesContentHash`. One ping can say “geometry changed” without a new Hot blob. Apply **merges** by id + `updatedAt` (union — a tablet subset must not wipe Linux-only paddocks) and must not flip map `isLoaded`. Viewport pans do not PUT. Firestore outbox stays off. See [`SETTINGS_SYNC_AND_CREW.md`](SETTINGS_SYNC_AND_CREW.md) §9.

**Decision — 2026-09-14 (issue photos, compress + HotKey blob).** Field-issue photos are still one product. Both pipes compress with the same helper (`src/lib/photoCompress.ts`): longest edge **1600 px**, JPEG quality **0.72** (steps down to 0.40), hard cap **600 KB** after compress — reject if still over. Hosted uploads **only** that JPEG to the existing Storage path `farms/{farmId}/issues/{issueId}/photo.jpg` (no original + compressed). Freenet-native farms PUT one HotKey-sealed blob per photo (`mist/v1/farm/{id}/hot/photo/{issueId}`) plus a small photo index; the 20 s Hot-watch ping grows optional `photoIndexUri` / `photoIndexHash` and the other terminal GETs **only** new photo hashes. Do not republish Hot on every photo. Do not re-enable the Firestore outbox. Crew never need FarmSeed. Splitfile and WebP stay deferred. Diary photos are the same-day decision below. See [`SETTINGS_SYNC_AND_CREW.md`](SETTINGS_SYNC_AND_CREW.md) §9 and [`FREENET_OPERATOR_FLOW.md`](FREENET_OPERATOR_FLOW.md) §9.4.

**Decision — 2026-09-14 (Hot apply: issues + satellite).** Incoming Hot/Bones **merge** issues and diary (union by id + `updatedAt`) and must not remount the map. Watch treats a new Hot URI as a change even when the content hash matches (Send mints a new CHK). Freenet farms hydrate issues from `pufom_farm_local` + the local issue cache — they do not replace the list from empty Firestore. The satellite vs street choice stays on the device (`pufam.mapLayer.v1.{farmId}`); Bones may carry an explicit newer `mapLayer` + `mapLayerUpdatedAt`, otherwise apply keeps the local layer and the Landgate tile URL. Do not re-enable the cloud outbox. FarmSeed stays owner-only.

**Decision — 2026-09-14 (photos: keep path, ≤5 per issue, diary/events, record metadata + stable file id, flat export + Freenet-miss warning).** Keep the hosted issue folder; do **not** rename `photo.jpg` (first/legacy). Additive files only: `{photoId}.jpg` under the same issue folder, max **5** per issue. Diary photos: yes — wire Firestore **`events`** (display “diary”), Storage `farms/{farmId}/events/{eventId}/{photoId}.jpg`, Freenet `mist/v1/farm/{id}/hot/photo/event/{eventId}/{photoId}` (HotKey only, never FarmSeed). Who / where / when live on the record (`createdAt`, `createdBy`, optional `blockId`; `directedAt*` only when the diary/highlight already has an assignee). File names are stable opaque ids (`{issueId}_{photoId}.jpg` / `{eventId}_{photoId}.jpg`) so a later shed preSQL server can key without parsing paddock or people names. Export zip stays flat; if the photo index / `hasPhoto` says a JPEG exists but `pufam_issue_photos` has no bytes, **warn** (season review / hand-off) — do not silently omit. Same compressor and 600 KB cap; upload only the compressed file. One photo index still drives the 20 s watch (`photoIndexHash`); extra issue and event slots upsert by `(kind, recordId, photoId)` without republishing Hot. [`NAMING.md`](NAMING.md) §8.

**Decision — 2026-09-15 (issue photos: parts, not one 600 KB PUT).** Pack PUT is **64 KiB** (`FREENET02_MAX_BLOB_BYTES`); Hot AEAD hex roughly doubles plaintext, so a 600 KB JPEG cannot be one blob. **Planned, not shipped:** HotKey-sealed content-addressed parts + the existing photo index. Do not crush the product compressor. Do not implement Freenet splitfiles. Do not use BonesKey or FarmSeed. Do not remount the map. Home: [`FREENET_ISSUE_PHOTOS.md`](FREENET_ISSUE_PHOTOS.md).

**Decision — 2026-09-12 (in-app node, no second app).** AppImage starts the bundled linux-x64 0.2.135 node when a Freenet farm is open (reconciler + persist `desktop-prefs.mistEnabled`); `MIST_FREENET=1` stays a workshop override. APK starts an in-APK `android-arm64` node from the isolated `:freenet` service when the pack is enabled; attach-if-port-taken still wins if `:7509` is already bound. Capability `'android'` stays gated on a **live** `:7509`. Official GitHub still has no Android asset — build via `scripts/build-freenet-android.mjs`. Do not start a node on hosted web. `linux-x64` pin flipped to `verified` after the AppImage↔tablet highlight ping.

**Decision — 2026-09-13 (AppImage hosts like the APK).** `vendor/freenet/linux-x64/` 0.2.135 was already the `electron-builder` extraResource (`resources/freenet/`). Join, Send, Sync, and the join-ticket gate now call `ensureFreenetHostListening()` so a packaged AppImage starts (or attaches to) that binary without `MIST_FREENET=1` and without pairing a LAN hub. Electron login offers Freenet whenever the shell is desktop — the FarmCode / create path is the opt-in, not a Settings toggle. Attach-if-port-taken if `:7509` is already up; do not kill a third-party node.

**Decision — 2026-09-14 (Settings Freenet ring).** Settings → Sync shows Freenet node status and a small-world ring on APK, AppImage, and hybrid farms (`FreenetStatusCard`). Hidden on hosted-only farms with no pack, no mist session, and no live node. Hosted web with a Freenet farm: “Freenet runs on desktop/tablet, not in the browser.” Status is starting / listening on :7509 / On Opennet / attached (third-party) / failed — never “connected to LAN hub” when the in-app node is up. Ring uses peer locations when the node JSON has them; otherwise N count-only dots. **0.2.135 leftover:** no JSON peer-location route (`GET /status` 404; dashboard is HTML — do not scrape; `GET /v1/version` is version only). Peer count N is the latest `ring_connections=` / `connection_count=` in this bake’s `--log-dir`, not JSON. N≥1 → On Opennet + N dots; N=0 or no line yet → Listening + “joining / no ring peers in the log yet.” Poll 8 s while the section is visible. FarmSeed stays owner-only.

**Decision — 2026-09-14 (Settings Freenet traffic).** The ring stays quiet until this node PUTs or GETs. Animate **this node ↔ network** (short arc + spoke), or a line to a named peer when the log has `peer=` that matches a ring dot — never invented hops. Signals: (1) host/transport put/get/slot callbacks (`FreenetHostPlugin` + pack transport), labelled Sent/Fetched Hot, Bones, watch, photo, invite when the mist key or `trafficKind` is known; (2) the same `--log-dir` tail as `ring_connections=`, only `process_client_request` and non-relay `operations::put`. Skip HTML, `NEIGHBOR_HOSTING`, GET/PUT/SUBSCRIBE/UPDATE relay, and auto-fetch. Last N events fade in ~4 s. Same 8 s poll; do not scrape the dashboard.

**Decision — 2026-09-14 (attach only if Freenet 0.2).** Attach-if-port-taken requires a Freenet 0.2 identity (`GET /v1/version` `{ version }` matching 0.2, or a WebSocket hello on `/v1/contract/command`). TCP on `:7509` is not enough — a leftover HTML dashboard or other listener is not attached. Do not kill a verified Freenet Android Node or third-party Freenet. If `:7509` is occupied by something that is not Freenet, try to start the bundled node; if the port stays taken, fail clean and show it on Settings → Sync. USB/adb does not bind 7509 (adb is 5037; this repo never `adb reverse tcp:7509`).

**Decision — 2026-09-14 (start on farm open / Settings card).** Opening a Freenet farm (eager `farmSession` reconciler) **or** opening Settings → Sync Freenet card must `ensureFreenetHostListening()` — start the bundled node, or attach after `/v1/version`. Do not wait for Send / Join / Sync. A sealed mist session still wants a node when `farmStoreBackend` still says firebase. If `:7509` is already Freenet 0.2 (stale AppImage or Freenet Android Node), attach and tell the operator; do not kill a verified third-party node. If nothing valid is listening, start the bundled binary / `libfreenet.so`.

**Decision — 2026-09-14 (ask before cutting Freenet).** Quit / session-leave asks Keep running vs Stop Freenet. Stop only `managed` / `starting` (this bake’s child or `:freenet`). `attached` is left. Default Keep. Desktop: last-window / `before-quit` modal. Android: Settings **Stop Freenet on this device** + Sign out / Leave farm (swipe-away cannot ask). Never signal `org.freenet.androidnode`.

**Decision — 2026-09-12.** **FarmSeed stays only on owner devices** (paper FarmCode + owner machines). Owner recover is **not** a join. **Crew type only an invite** (`PUF-` / grant) — never a FarmCode. The invite unwraps a **weaker read capability** (HotKey / BonesKey, or equivalent — [`reference/MIST_NETWORK_STORAGE.md`](reference/MIST_NETWORK_STORAGE.md) § Invitation names), not FarmSeed. FarmSeed must **never** ride on a short 40-bit `PUF-` ticket. The 2026-09-10 §8 row that left InviteToken “not adopted” is superseded for **crew join only**. **Implemented 2026-09-12:** 26-symbol InviteToken locates a public crew-join slot (`HKDF` infos `freenet-crew-join-slot` / `-key` / `-envelope`); envelope `v: 3` `kind: 'crew-join'` wraps Hot/Bones only. Send no longer tells anyone to read out the paper FarmCode. Hole 4 still applies — revoke ≠ kick for data already fetched; do not fake kick ([`FREENET_OPERATOR_FLOW.md`](FREENET_OPERATOR_FLOW.md) §8). Hosted web still cannot run a Freenet node (decision 5). Join UX home: [`LOGIN_JOIN_SINGLE_BOX.md`](LOGIN_JOIN_SINGLE_BOX.md) **Decision — 2026-09-12**. Do not renumber [`SETTINGS_SYNC_AND_CREW.md`](SETTINGS_SYNC_AND_CREW.md) or any `Plans/reference/*` section. Experimental Freenet; shipping path remains Firebase Auth + invite PIN.
...
| Post-sign-in prompt | `FreenetHybridJoinPrompt` (`postSignInPrompt` surface) in `Layout` after a PIN lands in a hybrid farm with no seed on this desktop | Dismissible; opens `FreenetEnterFarmCode`. Web renders null (no capability). Added 2026-09-11 (`LOGIN_JOIN_SINGLE_BOX.md`) |
...
**Hermetic part built 2026-09-11.** Live checks not yet run — see the unticked items.

- [x] Wire `BrowserFreenetPutClient` / `BrowserFreenetSlotClient` into Hot, bones and slot publish inside the Electron adapter. `Freenet02WsTransport.putBlob` now calls `BrowserFreenetPutClient.putPackBlob` (so every caller of the transport — host wire, Express relay, `FreenetMistStore` outbox — publishes natively); slot publish is `putJoinSlotNative` (`units/mist-freenet/src/freenet02-slot-publish.ts`) → `BrowserFreenetSlotClient.putJoinSlot`, reached from `server/freenetSlotOps.ts` `nativeJoinSlotPut()` by both the host wire and the relay routes. Upsert = PUT, then `UpdateData::State` with the real code hash on "already exists". The clients take an injected `webSocket` constructor (Node 22 / Electron supply `globalThis.WebSocket`); the ciphertext guard stays on every put path. Hermetic proof: `units/mist-freenet/freenet02-native-wire.test.ts`.
- [x] Remove `fdev` from `scripts/freenet-binaries.json`, `electron-builder.yml` `extraResources`, `verify-desktop-deps.mjs`; delete `freenet02-fdev-*.ts`, `fcp-freenet-transport.ts`, `fcp-protocol.ts`, and the Hyphanet-era headers in `mistFreenetRoutes.ts`, `freenetPeerHost.ts`, `freenet-mist-store.ts`. Also gone: `FREENET_TRANSPORT`, `FDEV_BIN`, `PUF_FDEV_BIN`, `FREENET_FCP_*` (`NAMING.md` §3), `FDEV_BINARY` in `puf-freenet-host`, the fdev-based `freenet02-slot-live.test.ts`. `verify-pack-contract.mjs` computes the code hash itself (BLAKE3 of the unwrapped module) and `build-slot-contract.mjs` is cargo-only — it writes the 40-byte package header itself and reproduces the pinned artifact byte-for-byte. `slotContract.builtWith.fdev` stays in the manifest as the historical record of what packaged the pinned copy.
- [x] Re-pin node version: **0.2.135** (`v0.2.135`, checksums from the release's `SHA256SUMS.txt`, `freenet --version` → `0.2.135 (ea1ff5f169bc)`), linux-x64 and win-x64. Slot contract **not** rebuilt (see decision 4) — re-pin only if the 0.2.135 node rejects the pinned WASM. The cross-version check *is* the smoke: `npm run mist:smoke:native` (`scripts/smoke-freenet-native.ts`) starts the vendored node on a temp data dir, runs the four `FREENET_LIVE_WS=1` suites (`native-put-live`, `native-slot-live`, `browser-get-live`, `freenet02-live` round-trip) against it, stops it, and prints one verdict with the node version.
- [x] **linux-x64 verified** against 0.2.135 (2026-09-12): AppImage↔tablet highlight ping exercised the bundled node; manifest `verified`. `npm run mist:smoke:native` remains the hermetic live suite if you want it again.
- [ ] Verify the win-x64 binary (`unverified` in the manifest); first-launch on Windows.
- [ ] A→B desktop smoke through the native path (was through `fdev` on 2026-08-04).
...
- [x] aarch64 `libfreenet.so` **workshop-built 2026-09-12** from pinned `v0.2.135` (`scripts/build-freenet-android.mjs`, NDK 28.2, rustc 1.94.0). Official GitHub still has no Android asset. Isolated `:freenet` service **spawns** when the so is present, fails clean when absent, attach only if `:7509` is Freenet 0.2. Binary is gitignored (~56 MB).
...
- [x] **2026-09-14 (partial):** tablet SM-T545 → phone SM-S911B + Linux AppImage Bones paddock sync on Opennet (Clare Downs; hours-old paddocks after Linux adopted the Freenet session). Logged in [`FREENET_OPERATOR_FLOW.md`](FREENET_OPERATOR_FLOW.md) §8. Bake **0.0.2**. Not a clean-install scripted pair.
...
- [ ] Hole 5 closed in §8; E-08 marked done in ROADMAP. **Next (2026-09-15):** [`DAY_RUN_2026_09_15.md`](DAY_RUN_2026_09_15.md).
...
| InviteToken / JoinEnvelope / invite index | `reference/MIST_NETWORK_STORAGE.md` § Invitation | **Decision — 2026-09-12, implemented 2026-09-12:** crew join is a 26-symbol InviteToken wrapping Hot/Bones (not FarmSeed). Frozen `§` titles in the reference doc are unchanged |
