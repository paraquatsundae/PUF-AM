...
- [ ] **linux-x64 verified** against 0.2.135: run `npm run desktop:vendor && npm run mist:smoke:native`, then flip `linux-x64` from `pending-live-check` to `verified` in the manifest. Not run on 2026-09-11 (hermetic-only session).
...
| InviteToken / JoinEnvelope / invite index | `reference/MIST_NETWORK_STORAGE.md` § Invitation | **Decision — 2026-09-12:** crew join is InviteToken-class wrap of Hot/Bones (not FarmSeed). FarmCode + short ticket as the *crew* handoff is superseded. Design not implemented; frozen `§` titles in the reference doc are unchanged |
