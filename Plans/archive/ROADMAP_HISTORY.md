# ROADMAP history — Steps 1–13 detail (Phases A–C)

> **Archived 2026-09-10.** Moved out of [`../ROADMAP.md`](../ROADMAP.md) when the completed phases were condensed into a dated summary. Content below is the Steps 1–13 detail as it stood on 2026-09-10 (all thirteen steps completed 2026-07-13), plus the 2026-07-13 progress-log rows. Every still-open `[ ]` item in these steps is **also** listed verbatim in `ROADMAP.md` § Open leftovers from Steps 1–13; this file is history, not a tracker. Section links here were rewritten for the archive location only.

**Phases as originally defined**
...
- [x] Record results in `Plans/archive/SMOKE_TEST_LOG.md` (pass/fail per route)
...
- `Plans/archive/SMOKE_TEST_LOG.md` (new)
...
- [x] Run `npm audit` and save full report to `Plans/logs/AUDIT_LOG.md`
...
**Result:** 34 → 2 vulnerabilities (0 critical, 1 high, 1 low). See `Plans/logs/AUDIT_LOG.md`.
...
- [ ] Debounce before refetch. `useOrchardMapViewport.ts:126` debounces `moveend`/`zoomend` at 500 ms (not 300 ms in `mapStore.ts`), but `setBounds` only stores the box — no refetch, and `orchardMapLayerSync.ts` never reads it. **Nothing is viewport-culled** — design in [`archive/MAP_VIEWPORT_CULLING.md`](MAP_VIEWPORT_CULLING.md); measure with the guard before building.
...
## Progress log rows moved here (2026-07-13)
