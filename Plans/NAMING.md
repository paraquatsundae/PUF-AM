...
**Two “plugin” words:** UI **Settings → Plugins** is the operator home for crop packs, the **farm pack** (`farm_feed`, under General), and the Freenet **network pack**. Docs still distinguish seams. Never implement a crop pack as a Freenet unit (or the reverse). Messaging is not a crop.

| UI | Meaning |
|----|---------|
| **Settings → Plugins** | Install/select crop packs; farm pack (`farm_feed`) under General; Freenet network pack under Network & storage |
| **Crop pack** / **pack plugin** | Optional farm capability (catalog + modules); zip Install |
| **Farm pack** (plugin kind) | Docs/schema only. Settings tile says **Farm feed** (`farm_feed`). Do not reuse “farm pack” in that UI — `.pufom` already owns the phrase |
