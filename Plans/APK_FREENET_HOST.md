...
**2026-09-13:** Debug APK probe on the main thread threw `NetworkOnMainThreadException`; fail-clean stored `no android-arm64 binary` while `libfreenet.so` was extracted. Probe/bring-up now run off the main thread; a Freenet farm must not be sent to a LAN hub as the only path.
**2026-09-14:** three-device Bones paddock sync on Opennet (SM-T545 write → SM-S911B + Linux AppImage) — [`FREENET_OPERATOR_FLOW.md`](FREENET_OPERATOR_FLOW.md) §8. Bake **0.0.2**. Hole 5 advanced, not closed. **Next (2026-09-15) — two tracks:** codebase health + security review, then issue photos over Freenet (packet size; photos not shipped); **parallel:** chill portions pack + weather/DPIRD wiring stale — [`PLUGIN_AUTHORING.md`](PLUGIN_AUTHORING.md) § Template pack.
