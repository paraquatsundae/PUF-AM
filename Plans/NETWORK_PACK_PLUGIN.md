...
| `loginJoin` | `Login.tsx` step `join` · freenet | Gets `{ code; kind: 'farm-code' \| 'join-ticket'; heldTicket?; availability: 'host' \| 'reader'; onBack() }`. Must not render `code`. FarmCode-first; pre-warms the node (`freenetLoginPrewarm.ts`) |
...
| `postSignInPrompt` | `Layout.tsx` beside `CloudMirrorBanner` | Dismissible; gates itself. Hybrid: enter the FarmCode after a PIN sign-in |
...
| Android APK | `'android'` when the Capacitor `FreenetHost` plugin is registered **or** a node answers on `127.0.0.1:7509` (attach to Freenet Android Node). [`APK_FREENET_HOST.md`](APK_FREENET_HOST.md) | plugin lifecycle + page WS PUT; `:freenet` process when we have a binary |
| Hosted web | `null` | none — a browser cannot run a node |

The capability is the **only** thing the pack asks about the shell. It is not `VITE_MIST_EXPERIMENTAL`, not `MIST_FREENET`, not a hostname. The login option keys off it too (`freenetOptionState`, decision 5): `'electron'` → available or *needs-setting*; `'android'` → available; `null` → hidden, except the `npm run dev` workshop hub (which *is* the sidecar) and an APK with the mist gate open but no host yet, which still reads through a paired hub ([`reference/APK_FREENET_PLUGIN.md`](reference/APK_FREENET_PLUGIN.md) §7).
...
The slot is the join ticket's mutable address (`Plans/reference/MIST_NETWORK_STORAGE.md`); the page builds and signs the `PUFSLOT1` state, the host puts it. Since Phase 2 (2026-09-11) every put on the host is **native over the node's WebSocket**: `putCiphertext` goes through `Freenet02WsTransport.putBlob` → `BrowserFreenetPutClient`, and `putSlotState` through `putJoinSlotNative` → `BrowserFreenetSlotClient` (PUT, then `UpdateData::State` with the real code hash when the slot already exists). No CLI is spawned and no second binary is bundled; the interface above did not change when that went native.
...
- **Node per device.** One node, however many farms. The reconciler (`freenetHostReconcile.ts`, mounted by `useFreenetHostReconciler` from the `farmSession` surface) computes `want` through `freenetHostWant.ts` — Freenet-native farm or mirror device: `farm open ∧ local flag ∧ capability is electron or android`; hybrid member: `farm open ∧ farm-doc enabled ∧ seed on this device for this cloud farm ∧ capability is electron or android`; plain cloud farm: never — and moves the node toward it, debounced (1.5 s) so switching between two enabled farms never restarts it. On Android, `start` attaches to `:7509` if a node is already up. The login join step pre-warms the same reconciler (`prewarmFreenetHost`) so the node is connecting while the name is typed; core never imports pack code or mentions a node.
...
`tests/joinCodeClassifier.test.ts`, `tests/joinCodeFlow.test.ts`, `plugins/freenet_host/src/freenetLoginPrewarm.test.ts`, `plugins/freenet_host/src/shouldOfferFarmCodePrompt.test.ts` — FarmCode-first login join (`LOGIN_JOIN_SINGLE_BOX.md`).
