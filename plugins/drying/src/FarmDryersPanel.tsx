...
export function FarmDryersPanel({ onSaved }: { onSaved?: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Editing is locked while this is set. The list on screen would be
        // empty, and saving it back would wipe the farm's real dryers.
  ...
  const handleSave = async () => {
    if (!farmId || !canEdit || loadFailed) return;
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={saving || !farmId || !canEdit || loadFailed}
      ...
      {loading ? (
        ...
        <div className="py-3 px-3 rounded-lg border border-amber-200 bg-amber-50 space-y-2">
          <p className="text-xs font-semibold text-amber-900">Could not load the dryer list.</p>
          <p className="text-[11px] text-amber-800">
            Editing is off until it loads. This farm may already have dryers saved, and saving
            now would replace them with an empty list.
          </p>
          <button
            type="button"
            onClick={() => setReloadKey((n) => n + 1)}
            className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-900 px-2 py-1 rounded-lg border border-amber-300 bg-white hover:bg-amber-100"
          >
            Try again
          </button>
        </div>
