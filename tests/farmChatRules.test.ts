...
describe('firestore.rules farm_chat/log', () => {
  it('caps the rolling doc and refuses a messages collection', () => {
    ...
    expect(rules).toContain('data.messages.size() <= 5');
  ...
  it('members read; admin/farmer write; no photo fields on the allow-list', () => {
    ...
    expect(helper).toContain("hasOnlyAllowedFields(['messages', 'updatedAt', 'updatedBy', 'dayDate', 'dayBlob'])");
    expect(rules).toContain('match /farm_chat/archive_index');
    expect(rules).toContain('match /farm_chat_archives/{dayId}');
    expect(rules).toContain('allow read: if isFarmAdmin(farmId)');
