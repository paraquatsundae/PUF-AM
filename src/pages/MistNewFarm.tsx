...
import { createAppFarmStore } from '../mist/createFarmStore.ts';
...
export function MistNewFarm() {
  ...
  const finishSetup = async () => {
    ...
    try {
      ...
      await saveMistDeviceSession(session, skipPin ? undefined : devicePin);
      createAppFarmStore(parsed.farmId);

      // Full reload so AuthContext picks up mist session without Firebase.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4">
        <div className="max-w-lg w-full space-y-6 bg-white p-8 rounded-2xl shadow-xl border border-amber-200">
          ...
          <code className="block font-mono text-sm sm:text-base leading-relaxed tracking-wide text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-4 break-all">
            {farmCode}
          </code>
