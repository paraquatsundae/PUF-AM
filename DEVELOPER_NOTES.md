...
**Status:** workshop bench pass on two laptops, not a paddock trial. Mist stays experimental behind `VITE_MIST_EXPERIMENTAL` + the mist backend toggle; **Firebase Auth + invite PIN remains the shipping path**. Known rough edges going into Phase 4: mist still needs `MIST_FREENET=1` on the launch (or Start Freenet node in Settings), the join ticket is a manual copy/paste because pack-contract URIs are immutable (Option B), and the Opennet bootstrap wait is real. See [`Plans/MIST_TWO_FEDORA_FREENET.md`](Plans/MIST_TWO_FEDORA_FREENET.md) § AppImage A→B and [`Plans/DESKTOP_FREENET_PLUGIN.md`](Plans/DESKTOP_FREENET_PLUGIN.md) §14 Phase 3/4.
