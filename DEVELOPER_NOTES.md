...
Durable write-up of start / send / join / People as the code stands: [`Plans/FREENET_OPERATOR_FLOW.md`](Plans/FREENET_OPERATOR_FLOW.md). Known holes and the order we will address them: [`Plans/FREENET_HOLES.md`](Plans/FREENET_HOLES.md). In-app **How this works** (same body as login) lives in [`src/components/FreenetHowItWorks.tsx`](src/components/FreenetHowItWorks.tsx) — button on Settings → Sync, Farm setup → People, and the join-ticket gate.
...
**Network pack in the APK (E-08):** operator name for Freenet under Settings → Plugins → Network & storage. Code `kind` stays `system` (`freenet_host`) — not a crop-pack zip. Implementation: isolated `:freenet` process + native PUT (no `fdev`). Plan: [`Plans/APK_FREENET_HOST.md`](Plans/APK_FREENET_HOST.md).
