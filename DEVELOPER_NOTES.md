...
Quick map: operators see **PUF-AM** (`src/brand.ts`); wire/sync stays **PUFOM** (`.pufom`, `pufom_*`, `_pufom-sync._tcp`); Android **`com.sentinut.farm`** and **`sentinut_*`** storage are frozen for continuity. Rebrand checklist: [`Plans/archive/RENAME_TO_PUFAM.md`](Plans/archive/RENAME_TO_PUFAM.md).

**Crop packs / Plugins** (Settings → Plugins; Install / Activate / Deactivate / Delete): **how-to** [`Plans/PLUGIN_AUTHORING.md`](Plans/PLUGIN_AUTHORING.md) · contract [`Plans/CROP_PACK_PLUGIN.md`](Plans/CROP_PACK_PLUGIN.md) · catalog `shared/farm/cropPacks.ts` (reads `plugins/<id>/`) · pack code `plugins/<id>/src/`, discovered by `src/packs/registry.ts` at build time — a pack's UI wires itself, though its adapter / module id / catalog row in `shared/` are still hand-added · zip drop `plugins/` (`npm run plugins:verify|unpack|pack|list`) · Settings UI `src/components/PluginsPanel.tsx` · PR [`.github/PULL_REQUEST_TEMPLATE/crop-pack.md`](.github/PULL_REQUEST_TEMPLATE/crop-pack.md). Freenet is listed under Network & storage (not a crop-pack Install).
...
- **2026-09-11 — FarmCode-first login join.** `/login` lands on one box (`Plans/LOGIN_JOIN_SINGLE_BOX.md`). A ticket typed first is held (session draft, ticket only); the FarmCode is never in the ticket. Hosted web refuses Freenet codes (decision 5).

- Full plan: [`Plans/reference/MIST_NETWORK_STORAGE.md`](Plans/reference/MIST_NETWORK_STORAGE.md)
- **Phase 1 contract (frozen API, historical):** [`units/mist-freenet/`](units/mist-freenet/) — types + `MistStore` + `FarmStoreAdapter`. In-app wiring started at Phase 4.
...
- **Phase 4 app wiring (done):** FarmCode (`mist-fc-2` — 17 typed symbols / 80 bits; legacy 27-symbol `mist-fc-1` still recovers), `src/mist/` FarmStore factory + mist first-run at `/login/mist-new-farm`, Settings bones workshop. Default backend remains Firebase.
...
- **Phase 8 two-laptop smoke (done, ~2026-08-03):** Pre-Freenet two-laptop pass — Laptop A create + local Hot; Laptop B **FarmCode recovery** → same `farmId` on localhost; bones/Hot per-device (expected). See [`Plans/archive/MIST_TWO_LAPTOP_SMOKE.md`](Plans/archive/MIST_TWO_LAPTOP_SMOKE.md).
...
- **Phase 10 two-Fedora Freenet (workshop, Option A):** Cross-laptop Hot + bones via **join ticket** (`{ hotUri, bonesUri }`). See [`Plans/reference/MIST_TWO_FEDORA_FREENET.md`](Plans/reference/MIST_TWO_FEDORA_FREENET.md). Option B (deterministic mutable contract) deferred.
...
- **Phase 11h short join tickets over LAN (done ~2026-08-04):** The join handoff was a JSON blob of two `FN02@…` URIs — fine with a clipboard, useless read off a whiteboard onto a phone, which is the handoff that actually happens. Laptop A now mints **eight Crockford Base32 symbols** (`PUF-K7M2-9Q4X`, 40 bits) alongside the publish and registers a **join manifest v2** — `{ v: 2, farmId, hotUri, bonesUri, role, permissions?, expires?, ticket }` — on its own LAN hub ([`shared/sync/joinTicket.ts`](shared/sync/joinTicket.ts), [`server/joinManifestStore.ts`](server/joinManifestStore.ts), [`server/joinTicketRoutes.ts`](server/joinTicketRoutes.ts)). Laptop B recovers with the FarmCode and lands on a **blocking** *Enter join ticket* screen ([`plugins/freenet_host/src/MistJoinTicketGate.tsx`](plugins/freenet_host/src/MistJoinTicketGate.tsx)) instead of an empty farm plus a scavenger hunt through Settings. **Freenet still carries every byte of farm data**; the LAN only carries the addresses, and a ticket is a capability rather than a key — the blobs it points at are AEAD-sealed under a FarmSeed-derived key, so it never substitutes for the FarmCode. Resolution happens **server-side in Node** (own shelf → owner-address hint → mDNS peers) because a page on `https://am.pufworks.farm` cannot fetch `http://192.168.x.x` without being blocked as mixed content; that also keeps CORS out of it. Roles use the mist vocabulary **`owner | admin | farmer | viewer`** (not `worker`), default `farmer`, with `permissions` reserved on the manifest for the grants the four names will not cover. `role` is an authority label, not a crypto boundary: it decides that only an `owner` is marched through `/farm-setup` ([`mistSetupDestination`](src/mist/finishMistFarmSetup.ts)) while a joiner's geometry arrives with the farm. Ticket → manifest sits behind [`JoinTicketResolver`](src/mist/joinTicketResolver.ts); `LanJoinTicketResolver` runs first and `FreenetSlotJoinTicketResolver` (below) is the fallback. **Known gap at the time:** the Electron shell bound loopback only and did not advertise on mDNS, so two AppImages could not discover each other — the joiner typed the owner's LAN address, or fell back to the raw FN02 ticket, which survives under *Advanced* on both cards. Tests: [`tests/joinTicket.test.ts`](tests/joinTicket.test.ts), [`src/mist/joinTicketResolver.test.ts`](src/mist/joinTicketResolver.test.ts), [`tests/api/joinTicketRoutes.test.ts`](tests/api/joinTicketRoutes.test.ts), [`src/mist/mistJoinRouting.test.ts`](src/mist/mistJoinRouting.test.ts).
- **Phase 11i storage choice on the start screen (done ~2026-08-05):** The two storage worlds were not equally visible: Freenet lived as two small “experimental” links at the bottom of the *Create a farm* tab, so an operator who wanted an offline farm had to first pretend to make a cloud one. `/login` now opens on a two-card chooser — **Cloud sync** (Firebase invite PINs, marked *Recommended*) and **Offline Freenet network** (*Experimental*, FarmCode + join ticket) — and the Freenet card leads to a second step with *Start a new farm* → `/login/mist-new-farm` and *Join a farm I already have* → `/login/mist-recover`. The chooser is deliberately gated by [`freenetOptionState()`](src/lib/loginStorageChoice.ts): mist on → both cards; a **desktop** shell with mist off → Freenet shown greyed pointing at Settings → *Farm sync between laptops*, because the node lives in that app; plain web / Capacitor → Firebase only, byte-identical to the production flow. Picking a card **does not** write `pufam.farmStoreBackend` — only [`finishMistFarmSetup()`](src/mist/finishMistFarmSetup.ts) does, so backing out of the Freenet path cannot strand a device on the mist FarmStore. A remembered Firebase device still lands straight on its PIN prompt ([`initialLoginStep()`](src/lib/loginStorageChoice.ts)), and the nearby-farms GPS lookup now waits for the cloud path to be chosen instead of firing behind the chooser. Tests: [`tests/loginStorageChoice.test.ts`](tests/loginStorageChoice.test.ts).
- **Phase 11j mist UI in the APK, honestly gated (done ~2026-08-05):** The tablet build had the same bug the AppImage had in Phase 11d and no way out of it. `VITE_MIST_EXPERIMENTAL` is inlined by Vite at build time, so `build:android` shipped an APK with the storage chooser dead-code eliminated — and Android has **no preload bridge**, so unlike desktop there is no runtime path that could bring it back. [`scripts/build-android-web.mjs`](scripts/build-android-web.mjs) is now the Android sibling of `build-desktop-web.mjs`: `VITE_CAPACITOR=1` plus the mist flag defaulted to `true`, with `--no-mist` for a Firebase-only APK. `npm run apk:debug` takes it the rest of the way — `cap sync` with `CAP_PACKAGED=1` (a live-reload APK points at `http://10.0.2.2:3000`, which is an emulator address and a blank screen on a real tablet) and `assembleDebug`. Verified: `app-debug.apk`, 7.3 MB, chooser and FarmCode flow present in `assets/public`. **Freenet is not in the APK and is not faked.** A tablet cannot host a node — no Node runtime, the whole Freenet client path is Express-side, Android will not `exec` a binary outside `nativeLibraryDir`, and PUT still needs `fdev` — so [`src/lib/freenetRuntime.ts`](src/lib/freenetRuntime.ts) answers *where is the node* once and everything reads it: `desktop-host`, `browser-sidecar`, `android-hub` (an APK built with `VITE_MIST_FREENET_API`, for the workshop), or `android-no-host`, which **removes the Connect button** and says Freenet lives on a PUF-AM laptop. Blocking beats letting it fail: `apiBase.ts` would send that fetch to `10.0.2.2:3000` and report a generic disconnect, which an operator in a paddock reads as bad signal. Phase 1 for tablets is the existing LAN path, not Freenet — see [`Plans/reference/APK_FREENET_PLUGIN.md`](Plans/reference/APK_FREENET_PLUGIN.md). Companion docs: [`Plans/archive/FREENET_CONTRIBUTE_AND_STORAGE.md`](Plans/archive/FREENET_CONTRIBUTE_AND_STORAGE.md) (contribute vs communicate, what is published, what is sealed), [`Plans/LOCAL_DATA_STORAGE.md`](Plans/LOCAL_DATA_STORAGE.md) (every local store across all four shells, authoritative vs cache). Tests: [`tests/freenetRuntime.test.ts`](tests/freenetRuntime.test.ts).
- **Phase 11k join slot — short tickets without the owner's Wi‑Fi (done ~2026-08-09):** Phase 11h's ticket only meant something to a hub on the owner's network. Lifting that needed a **Rust/WASM Freenet 0.2 contract**, the first compiled artifact this repo owns: [`units/mist-freenet/contracts/slot-contract`](units/mist-freenet/contracts/slot-contract). The obstacle was never Freenet's mutability — `ContractInterface` has `update_state` — it was the bundled **pack** contract's convention of `parameters = blake3(state)`, which makes an address a function of its content and is therefore circular for a lookup: a joiner holding only `PUF-K7M2-9Q4X` cannot compute an address that depends on the bytes it is trying to fetch. The slot contract puts a **derived slot id** in `parameters` instead, so `slot_id = HKDF(FarmSeed, "freenet-join-slot:" + ticket)` and `parameters = slot_id ‖ farm ed25519 pubkey` make the address `BLAKE3(code_hash ‖ parameters)` — computable by owner and joiner independently, by nobody else. **The FarmSeed is in the derivation on purpose:** a ticket overheard on its own points nowhere, and the network never sees a value derived from the ticket in the clear. State is a sequence-numbered, signed envelope around an AEAD-sealed `JoinManifestV2` the contract never reads; the verifying key lives in `parameters` rather than the state (otherwise anyone could put their own key at the same address), and `slot_id` is inside the signed message (otherwise a state signed for one ticket would verify in another ticket's slot, since both share a farm key). Signing and sealing happen **in the page**, so [`server/mistFreenetRoutes.ts`](server/mistFreenetRoutes.ts)'s two new routes are dumb byte movers that can neither read nor forge what they publish — and they inherit the existing paired-device token, since `/api/mist/freenet/` is already in `LAN_SCOPE_PREFIXES`. `FreenetSlotJoinTicketResolver` is **second** in `defaultJoinTicketResolvers()`: LAN still answers in milliseconds and still works with no internet, Freenet is what makes a ticket work three paddocks away. **Two pinned hashes, one artifact** — `SLOT_CONTRACT_CODE_HASH_B58` ([`units/mist-freenet/src/freenet02-slot.ts`](units/mist-freenet/src/freenet02-slot.ts)) and `slotContract.codeHashB58` ([`scripts/freenet-binaries.json`](scripts/freenet-binaries.json)) must both match the shipped WASM, because every slot address is derived from it; drift would let publishes succeed and land where nothing looks, so `npm run mist:build:slot` refuses to re-pin without `--accept-new-hash`. **What it does not lift:** the joiner still needs a Freenet node of its own and an unlocked device session, nothing prunes an expired slot the way a hub prunes its shelf (the joiner enforces `expires` after decrypting), and Opennet propagation still takes minutes after a send. Tests: 17 Rust unit tests including a byte-for-byte conformance check against the TypeScript encoder (`cargo test … --features contract`), [`units/mist-freenet/freenet02-slot.test.ts`](units/mist-freenet/freenet02-slot.test.ts), [`src/mist/joinSlotFreenet.test.ts`](src/mist/joinSlotFreenet.test.ts), [`tests/freenetVendorManifest.test.ts`](tests/freenetVendorManifest.test.ts), plus a live put/get against a real node (`FREENET_LIVE_WS=1 npm test -- units/mist-freenet/freenet02-slot-live.test.ts`).
- **Phase 11l join actually reachable — three bugs between the slot and a joiner (done ~2026-08-09):** Phase 11k built the slot; nothing could reach it. A tablet reported failing a join at `POST http://192.168.1.1205:3000/api/mist/freenet/peer/start`, and each layer of that one line was a separate defect. **(1) The Freenet resolver could never answer.** A slot address is derived from the FarmSeed a device PIN unlocks, and [`joinFarmWithShortTicket`](src/mist/mistJoinWithTicket.ts) forwarded `devicePin` to the rehydrate step but **not** to `resolveJoinTicket` — so `FreenetSlotJoinTicketResolver` failed with "unlock this device" on every PIN-locked device, which is the normal state after FarmCode recovery. Neither join surface collected a PIN at all. Both now do, and the PIN reaches the lookup ([`src/mist/joinTicketDevicePin.test.ts`](src/mist/joinTicketDevicePin.test.ts)). **(2) `peer/start` ran before every resolver and was allowed to fail the join.** [`MistJoinTicketGate`](plugins/freenet_host/src/MistJoinTicketGate.tsx) awaited a peer warm-up first, so an unreachable hub failed the join on the warm-up and no resolver ever ran — including the LAN one, which needs no Freenet. Worse, `mistFreenetFetch` defaults to a **300 s** ceiling, so a routable-but-silent hub bought a five-minute hang before any diagnosis. The start is now best-effort inside `try`/`catch`, skipped when `detectFreenetRuntime()` says there is no node, and the gate shows the honest no-host sentence instead of a `Connect Freenet` button. **(3) A remembered hub was never re-validated.** `pufom_last_sync_hub` survives `adb install -r`, so a value saved before the address field probed anything outlived its build; `getApiBaseUrl()` handed it to `fetch()` forever. The real hub was `192.168.1.205:3000` — the stored value had one extra digit, and an address that is not a URL rejects with the same bare `TypeError` as an unplugged laptop. Reads and writes now go through `normalizeHubBase()` ([`tests/apiBaseHubBase.test.ts`](tests/apiBaseHubBase.test.ts)). **Packaging was also incomplete:** `slot-contract.wasm` was not in `extraResources` and `FREENET_SLOT_WASM` was never exported, so a packaged desktop would have minted tickets whose slots it could not publish; and the slot's ed25519 signing added `@noble/curves`, which the asar allowlist did not carry — `desktop:verify:deps` refused to build, which is exactly its job. **Verified on hardware:** rebuilt APK on an SM-T545 now resolves the correct hub by NSD and polls `/api/mist/freenet/peer/status` against it; the mistyped address is gone. **What none of this changes:** a tablet still cannot host a Freenet node, and both the slot lookup and the Hot/bones pull are Express-side, so a tablet's route to a farm is always a hub — see [`Plans/reference/APK_FREENET_PLUGIN.md`](Plans/reference/APK_FREENET_PLUGIN.md) §7a for what "join from anywhere" does and does not mean on Android.
- **Network pack Phase 2 — native PUT everywhere, one binary (hermetic part done 2026-09-11):** Everything above that says "PUT goes through `fdev`" is history from this line on. [`Plans/FREENET_NETWORK_PACK.md`](Plans/FREENET_NETWORK_PACK.md) Phase 2 made the spike clients from `APK_FREENET_HOST.md` Phase 1 the *only* publish path: `Freenet02WsTransport.putBlob` now calls `BrowserFreenetPutClient` (so the host wire, the Express relay and the `FreenetMistStore` outbox all publish natively), and slot publish is `putJoinSlotNative` → `BrowserFreenetSlotClient` (PUT, then `UpdateData::State` with the real code hash when the slot already exists), shared by host and relay through `server/freenetSlotOps.ts`. The clients take an injected `webSocket` constructor rather than polyfilling a global — Node 22 and Electron both have `globalThis.WebSocket`. Deleted: `freenet02-fdev-put.ts`, `freenet02-fdev-slot.ts`, `fcp-freenet-transport.ts`, `fcp-protocol.ts`, the fdev slot live test, `FDEV_BINARY`/`PUF_FDEV_BIN` in the host unit, and the env vars `FREENET_TRANSPORT`, `FDEV_BIN`, `FREENET_FCP_*` ([`Plans/NAMING.md`](Plans/NAMING.md) §3). The node is pinned to **0.2.135** (`freenet --version` → `0.2.135 (ea1ff5f169bc)`); `fdev` is no longer fetched, pinned or bundled, so the vendor dir is one ~57 MB binary plus the licence. `verify-pack-contract.mjs` computes the code hash itself (BLAKE3 of the unwrapped module — what `fdev inspect` printed) and `build-slot-contract.mjs` is cargo-only, writing the 40-byte package header itself; a rebuild reproduces the pinned `slot-contract.wasm` byte-for-byte, so the slot hash was **not** re-pinned. New: `npm run mist:smoke:native` ([`scripts/smoke-freenet-native.ts`](scripts/smoke-freenet-native.ts)) starts the vendored node on a temp data dir, runs the four `FREENET_LIVE_WS=1` suites against it, and prints one verdict with the node version; `linux-x64` sits at `pending-live-check` in the manifest until that passes. **Not yet run:** that smoke, the Windows first launch, and the A→B desktop join through the native path. Hermetic proof: [`units/mist-freenet/freenet02-native-wire.test.ts`](units/mist-freenet/freenet02-native-wire.test.ts).
...
**Status:** workshop bench pass on two laptops, not a paddock trial. Mist stays experimental behind `VITE_MIST_EXPERIMENTAL` + the mist backend toggle; **Firebase Auth + invite PIN remains the shipping path**. Known rough edges going into Phase 4: mist opt-in is Settings (Phase 11f); `MIST_FREENET=1` remains a workshop override. The join ticket is still a manual handoff (Phase 11h shortened it to eight symbols and Phase 11k's slot contract removed the requirement to be on the owner's Wi‑Fi, but somebody still reads it out), and the Opennet bootstrap wait is real. See [`Plans/reference/MIST_TWO_FEDORA_FREENET.md`](Plans/reference/MIST_TWO_FEDORA_FREENET.md) § AppImage A→B and [`Plans/reference/DESKTOP_FREENET_PLUGIN.md`](Plans/reference/DESKTOP_FREENET_PLUGIN.md) §14 Phase 3/4.

#### Milestone — Hot loss / Freenet recovery (ws02, ~2026-08-03)

**Workshop smoke succeeded:** Settings → *Freenet loss / recovery smoke* — publish Hot to Freenet 0.2 (`FREENET_TRANSPORT=ws02`, WebSocket `localhost:7509`), wipe local `pufom_farm_local` + mist `hot/current`, pull ciphertext and rehydrate diary/issues. FarmCode and mist device session preserved. Operator-reported successful rehydrate of **3 diary** entries; Freenet Hot contract hash prefix **`a31a5a98…`**. See [`Plans/archive/MIST_TWO_LAPTOP_SMOKE.md`](Plans/archive/MIST_TWO_LAPTOP_SMOKE.md) § Single-laptop disaster-recovery smoke.
...
Team workshop captured the following before wiring live Freenet. Full detail: [`Plans/reference/MIST_NETWORK_STORAGE.md`](Plans/reference/MIST_NETWORK_STORAGE.md) § Pre-Freenet workshop decisions; unit notes: [`units/mist-freenet/README.md`](units/mist-freenet/README.md) Phase 8+.
...
- Cross-device Hot/bones sync via in-process Freenet client — **Hot publish path started** (server peer + workshop publish/pull; originally over Hyphanet FCP, since 2026-09-11 Freenet 0.2 WebSocket only); bones/manifest Freenet sync deferred
- Reticulum unit, invite join QR, `sealHotPeriod` app trigger

### Freenet operator flow (today)

Durable write-up of start / send / join / People as the code stands: [`Plans/FREENET_OPERATOR_FLOW.md`](Plans/FREENET_OPERATOR_FLOW.md). Known holes and the order we will address them: [`Plans/FREENET_OPERATOR_FLOW.md`](Plans/FREENET_OPERATOR_FLOW.md) §8 (merged from `FREENET_HOLES.md`, 2026-09-10). In-app **How this works** (same body as login) lives in [`plugins/freenet_host/src/FreenetHowItWorks.tsx`](plugins/freenet_host/src/FreenetHowItWorks.tsx) — button on Settings → Sync, Farm setup → People, and the join-ticket gate.

| | |
|--|--|
| Two secrets | Paper **FarmCode** (identity, shown once) + short **join ticket** `PUF-XXXX-XXXX` (handoff after Send). A cloud invite PIN opens neither. |
| Start | Welcome → How this works → farm name → write FarmCode → optional device PIN → `/farm-setup`. Nothing on Freenet until Settings → Sync → **Send**. |
| Add a person | Laptop only. Send mints a new ticket (default 7 days). Joiner already has the FarmCode, then types the ticket. Ticket lookup is LAN first, Freenet slot fallback; farm bytes always travel on Freenet. |
| People | That hub’s shelf. Revoke stops new resolves; a device that already joined keeps its FarmSeed copy. Two tablets with no laptop cannot sync. Roles are UI bookkeeping — FarmCode is the crypto boundary. |

Phase 11a–11l above is how the machinery was built. The operator-flow plan is what an agent should read before changing Send, join, or People.

**Network pack in the APK (E-08):** operator name for Freenet under Settings → Plugins → Network & storage. Code `kind` stays `system` (`freenet_host`) — not a crop-pack zip. Implementation: isolated `:freenet` process + native PUT (no `fdev`). Umbrella plan (desktop + APK, per-farm enable, hybrid for cloud farms): [`Plans/FREENET_NETWORK_PACK.md`](Plans/FREENET_NETWORK_PACK.md); Android host detail: [`Plans/APK_FREENET_HOST.md`](Plans/APK_FREENET_HOST.md).
...
**Authoritative plan:** [`Plans/ROADMAP.md`](Plans/ROADMAP.md) — phase summary, open leftovers, Phase D/E trackers and progress log. Steps 1–13 task breakdown, acceptance criteria and dependency graph: [`Plans/archive/ROADMAP_HISTORY.md`](Plans/archive/ROADMAP_HISTORY.md) (moved 2026-09-10).
...
| 3 | `STEP-03` | Smoke-test `npm run dev` against Firebase — log in `Plans/archive/SMOKE_TEST_LOG.md` | `done` |
...
| 8 | `STEP-08` | `npm audit` — remediate critical vulnerabilities; log in `Plans/logs/AUDIT_LOG.md` | `done` |
