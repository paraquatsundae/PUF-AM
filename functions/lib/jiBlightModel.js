...
exports.JI_SECONDARY_INOCULUM_COEFF = exports.JI_INCUBATION_MAX_DAYS = exports.JI_INCUBATION_MIN_DAYS = exports.DEFAULT_SH_BUDBREAK = exports.JI_INOCULUM_WINDOW_DAYS = exports.JI_INOCULUM_K = exports.JI_ACTION_THRESHOLD = exports.JI_WATCH_THRESHOLD = exports.JI_PUBLISHED = void 0;
exports.bandFromRisk = bandFromRisk;
exports.kFromInoculumLevel = kFromInoculumLevel;
exports.resolveBudbreak = resolveBudbreak;
...
exports.JI_WATCH_THRESHOLD = 0.01;
/** At/above: Action. */
exports.JI_ACTION_THRESHOLD = 0.05;
...
exports.JI_INOCULUM_WINDOW_DAYS = 28;
/**
 ...
 */
exports.DEFAULT_SH_BUDBREAK = { month: 9, day: 1 };
/** Coerce stored month/day into a usable budbreak; never throws. */
function resolveBudbreak(month, day) {
    ...
    if (Number.isNaN(m) || Number.isNaN(d))
        return exports.DEFAULT_SH_BUDBREAK;
    if (m < 0 || m > 11 || d < 1 || d > 31)
        return exports.DEFAULT_SH_BUDBREAK;
    if (d > new Date(2001, m + 1, 0).getDate())
        return exports.DEFAULT_SH_BUDBREAK;
    return { month: m, day: d };
}
/** Ji: infected sites become symptomatic 15–21 days later (delay distribution, Fig. 3). */
exports.JI_INCUBATION_MIN_DAYS = 15;
exports.JI_INCUBATION_MAX_DAYS = 21;
/** Neutral choice for the coefficient Ji leaves unspecified (see shared module). */
exports.JI_SECONDARY_INOCULUM_COEFF = 1;
...
function runJiBlightModel(weather, options = {}) {
    ...
    const windowDays = options.inoculumWindowDays === undefined
        ? exports.JI_INOCULUM_WINDOW_DAYS
        : options.inoculumWindowDays;
    const secondaryCoeff = options.secondaryInoculumCoeff ?? exports.JI_SECONDARY_INOCULUM_COEFF;
    const minLag = options.incubationMinDays ?? exports.JI_INCUBATION_MIN_DAYS;
    const maxLag = options.incubationMaxDays ?? exports.JI_INCUBATION_MAX_DAYS;
    ...
    const eruption = new Array(weather.length + maxLag + 1).fill(0);
    const out = [];
    for (let i = 0; i < weather.length; i++) {
        ...
        if (windowDays == null || i < windowDays)
            cumulativeRain += R;
...
function isShBudbreakDay(date, budbreak = exports.DEFAULT_SH_BUDBREAK) {
    const { month, day } = (0, perthDate_1.getPerthYmd)(date);
    return month - 1 === budbreak.month && day === budbreak.day;
}
/** First budbreak on or after rangeStart (Perth calendar). */
function defaultShBudbreakDate(rangeStart, budbreak = exports.DEFAULT_SH_BUDBREAK) {
    const y = (0, perthDate_1.getPerthYmd)(rangeStart).year;
    const thisYear = new Date(y, budbreak.month, budbreak.day);
    if (toLocalISOString(rangeStart) <= toLocalISOString(thisYear))
        return thisYear;
...
function runJiBlightSeries(startDate, endDate, weatherData, options = {}) {
    const budbreak = options.budbreak ?? exports.DEFAULT_SH_BUDBREAK;
