...
const metnoForecast_1 = require("./metnoForecast");
...
exports.refreshWeatherCache = (0, scheduler_1.onSchedule)({
    ...
    for (const station of STATION_ANCHORS) {
        try {
            ...
            let weatherData = {
                ...(snap.exists ? snap.data()?.weatherData : {}),
            };
            let historicBackfilledAt = snap.exists
                ? snap.data()?.historicBackfilledAt
                : undefined;
            const existing = bounds(weatherData);
            const needsHistoric = existing.dayCount < 60 || !existing.startDate || existing.startDate > histStart;
            if (needsHistoric) {
                console.log(`[refreshWeatherCache] Historic backfill ${station.stationCode} ${histStart} → ${recentEnd}`);
                const historic = await fetchStationWeather(apiKey, station.stationCode, histStart, recentEnd);
            ...
            try {
                ...
                const { forecastData, fetchedAt } = await (0, metnoForecast_1.fetchMetnoDailyForecast)({
                    lat: station.lat,
                    lng: station.lng,
                });
                ...
                console.log(`[refreshWeatherCache] ${station.stationCode}: forecast ${Object.keys(future).length} days`);
            }
            catch (fcErr) {
